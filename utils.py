...
def get_q_neighbors(vec, alpha_s=30):
    vec = np.array(vec)
    # angles = np.zeros((vec.shape[0], vec.shape[0]))
    for i in range(vec.shape[0]):
        angles = np.array([np.arccos(np.dot(vec[i, :], v).clip(-1, 1)) for v in vec])
        angles = np.rad2deg(angles)
        print(len(np.where(angles < alpha_s)[0]))
